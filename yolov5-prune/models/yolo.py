...
class Detect_DyHead_Prune(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), hidc=128, inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
...
class BaseModel(nn.Module):
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            if hasattr(self, 'fuse_repvgg_block'):
                m.fuse_repvgg_block()
        self.info()
        return self
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_DyHead_Prune, Segment)):
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, C3_Faster, RepConv, C2f, C2f_Faster,
                GSConv, GSConvns}:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3_Faster, C2f, C2f_Faster}:
        ...
        # TODO: channel, gw, gd
        elif m in {Detect, Detect_DyHead_Prune, Segment}:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5-dyhead-prune.yaml', help='model.yaml')
